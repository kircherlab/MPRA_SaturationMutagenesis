...
load_element_list <- function(path) {
...
enhancers <- load_element_list("data/enhancers.tsv")
promoters <- load_element_list("data/promoters.tsv")

# Combine the elements (for download)
elements <- bind_rows(enhancers, promoters) |> arrange(Element)

# Read the variant values!
data_table <- read.delim("data/elements.tsv.gz", quote = "") |>
  na.omit() |>
  arrange(Element, Chrom, Pos, Ref, Alt) |>
  mutate(Coefficient = round(Coefficient, 2), pValue = round(pValue, 5))
...
ui <- tags$html(
  ...
  tags$head(
    tags$meta(
      name = "description",
      content = "MPRA data access portal for browsing promoter and enhancer saturation mutagenesis results, visualizing variant effects, and downloading element datasets." # nolint: line_length_linter.
    )
  ),
  tags$body(
    fluidRow(
      ...
      column(
        ...
        tags$main(
          ...
          navbarPage("MPRA-Data",
            windowTitle = "MPRA Saturation Mutagenesis Data Access Portal",
            theme = "mpra.css",
            tabPanel(
              "About",
...
server <- function(input, output, session) { # nolint: cyclocomp_linter.
        ...
        sidebarLayout(
          ...
          mainPanel(
            ...
            tabsetPanel(
              ...
              tabPanel(
                "Variant plot", plotlyOutput(paste0(name, "_plot")),
  ...
  render_element <- function(name, release, barcodes, threshold, deletions, range, output, session, color_palette) {
    ...
    if (!is.null(release) && !is.null(barcodes) && !is.null(threshold)) {
      ...
      if (!is.na(threshold) && !is.na(barcodes) && threshold > 0 && barcodes > 0) {
        ...
        output[[paste0(name, "_table")]] <- DT::renderDT(data,
        ...
        # plot the data. here the helper modify_filterdata is used to filter the data (from plots.R
        # and the function get_plot from the same source to plot the ggplot
        plot_data <- modify_filterdata(data_general, barcodes, threshold, deletions, range)
        if (plot_data |> nrow() > 0) {
          isolate({
            output[[paste0(name, "_plot")]] <- renderPlotly({
              p <- get_plot(plot_data, name, release, color_palette)
              ggplotly(p) |>
  ...
  update_region_slider <- function(session, id, name, release) {
  ...
  session$userData$actual_selected_release <- list()
  ...
  for (name in promoters$Element) {
    session$userData$actual_selected_release[[name]] <- "GRCh37"
    isolate({
      appendTab("promoterNavigation", experiment_page(name))
      update_region_slider(session, paste0(name, "_region"), name, "GRCh37")
      render_element(name, "GRCh37", 10, 1e-5, TRUE, NULL, output, session, "default")
    })
  }
  for (name in enhancers$Element) {
    session$userData$actual_selected_release[[name]] <- "GRCh37"
    isolate({
      appendTab("enhancerNavigation", experiment_page(name))
      update_region_slider(session, paste0(name, "_region"), name, "GRCh37")
      render_element(name, "GRCh37", 10, 1e-5, TRUE, NULL, output, session, "default")
    })
  }
  ...
  observe({
    for (name in elements$Element) {
      ...
      p_value <- input[[paste0(name, "_pvalue")]]
      ...
      if (!is.null(release) && session$userData$actual_selected_release[[name]] != release) {
        session$userData$actual_selected_release[[name]] <- release
        update_region_slider(session, paste0(name, "_region"), name, release)
      }
      # update data table and plot
      range <- input[[paste0(name, "_region")]]
      isolate({
        render_element(name, release, barcodes, p_value, deletions, range, output, session, color_palette)
      })
    }
  })
  ...
  output$downloadData_selected <- downloadHandler(
    ...
    filename = function() {
      selected_elements <- c(input$download_promoter, input$download_enhancer)
      paste0(
        paste(input$download_reference_all, paste(selected_elements, sep = "_", collapse = "_"), sep = "_"),
        input$download_format_all
      )
    },
  ...
  output$downloadData_all <- downloadHandler(
    # function for filename
    filename = function() {
      paste0(paste(input$download_reference_all, "ALL", sep = "_"), input$download_format_all)
    },
    content = function(file) {
      release <- input$download_reference_all
      data <- data_table |>
